class Personne
  ...
  def subit_attaque(degats_recus)
	...
	puts "#{nom} à subits #{degats_recus} points de dégats !"
	if @points_de_vie <= 0
...
class Joueur < Personne
  ...
  def soin
    ...
    # - Gagner de la vie
    # - Affiche ce qu'il s'est passé
	@points_de_vie += rand(30..50)
	puts nom + " récupere des forces"
  end
  ...
  def ameliorer_degats
	...
	puts nom + " a améliorer son attaques de #{degats_bonus} pour le prochain tours !"
  end
end
...
class Jeu
  ...
  def self.est_fini(joueur, monde)
	...
	if joueur.points_de_vie <= 0 || monde.ennemis_en_vie.size == 0
		return true
	else
		return false
	end
